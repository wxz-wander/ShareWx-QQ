...
public class OtherApiActivity extends BaseActivity implements OnClickListener {
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.main_is_report_dau_btn: {
				QQMainActivity.mTencent.reportDAU();
			}
			break;
	...
	private void onClickOpenId() {
		if (QQMainActivity.ready(OtherApiActivity.this)) {
			UserInfo info = new UserInfo(this, QQMainActivity.mTencent.getQQToken());
	...
	private void onClickCheckLogin() {
		if (QQMainActivity.ready(OtherApiActivity.this)) {
			QQMainActivity.mTencent.checkLogin(new BaseUIListener(this));
			Util.showProgressDialog(OtherApiActivity.this, null, null);
		}
	}
				...
				if (ret == 0) {
					...
					if (mNeedReAuth) {
						Runnable r = new Runnable() {
							@Override
                            public void run() {
								QQMainActivity.mTencent.reAuth(activity,
